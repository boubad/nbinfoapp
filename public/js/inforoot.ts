...
export class InfoRoot {
	...
	public static add_item_to_array<T extends IElementDesc>(cont: T[], item: T): boolean {
    ...
    public static add_id_to_array(cont: string[], id: string): boolean {
		if ((cont == undefined) || (cont == null)) {
			cont = [];
		}
        if ((id === undefined) || (id === null)) {
	...
	public static add_array_to_array(cont: string[], oAr: string[]): void {
		if ((oAr !== undefined) && (oAr !== null)) {
			let n = oAr.length;
			for (let i = 0; i < n; ++i) {
				InfoRoot.add_id_to_array(cont, oAr[i]);
			}// i
		}// oAr
	}//add_array_to_array
